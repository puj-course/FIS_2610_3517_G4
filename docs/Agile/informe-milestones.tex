...
\begin{document}
\hypersetup{pageanchor=false,pdftitle={Informe de Postmortem - Milestone 4}}
\begin{titlepage}
\centering
\vspace*{0.5cm}
...
{\LARGE \textbf{Milestone 4: Dashboard, Alertas y Cierre del Sistema}}\\[0.3cm]
...
\clearpage
\section{Postmortem -- Milestone 4: Dashboard, Alertas y Cierre del Sistema}
...
\item El pipeline de CI/CD fue entregado funcional y completamente automatizado, con publicación condicional de imágenes Docker solo cuando el Quality Gate de SonarCloud es superado.
\item SonarCloud detectó el Quality Gate en estado \texttt{Failed} en la primera ejecución del pipeline, por Hotspots de seguridad no resueltos y cobertura de pruebas por debajo del umbral mínimo configurado.
\item La remediación de los Hotspots fue realizada de forma asíncrona por el equipo, demostrando el nivel más alto de cohesión técnica del proyecto.
\item Se detectaron Pull Requests enviados al repositorio sin validación funcional previa en local, confiando exclusivamente en que pasar el pipeline de CI implicaba que la lógica de negocio funcionaba correctamente. Esta práctica generó regresiones que no fueron detectadas por las pruebas automatizadas.
\item Los datos demo de prueba causaron inconsistencias en el flujo de alertas cuando no estaban emparejados correctamente con el \texttt{OWNER\_EMAIL} del usuario en sesión.
\item La navegación con componentes \texttt{Link} mejoró significativamente la experiencia de usuario al eliminar recargas completas de página entre vistas.
\end{itemize}

\subsubsection{Lecciones aprendidas}

\begin{itemize}[leftmargin=1.2em]
\item Superar el pipeline de CI es una condición necesaria, pero no suficiente para garantizar que la lógica de negocio funciona correctamente. La validación funcional humana en local sigue siendo irreemplazable.
\item La coherencia de los datos demo es un requisito de calidad, no un detalle de configuración: datos sin \texttt{OWNER\_EMAIL} correcto generan comportamientos impredecibles que pueden confundirse con bugs del sistema.
\item El comando \texttt{docker compose down -v} debe ser parte del protocolo estándar de limpieza del entorno de prueba para evitar que volúmenes persistentes defectuosos contaminen los resultados del QA.
\item La cobertura de pruebas debe crecer junto con el sistema, no añadirse como una actividad de cierre de milestone.
\end{itemize}
...
\bottomrule
\endlastfoot
1 & Estandarizar el uso de \texttt{docker compose down -v} como protocolo obligatorio de limpieza del entorno de prueba entre ciclos de QA. & Los volúmenes persistentes defectuosos contaminan el entorno de prueba y generan comportamientos inconsistentes difíciles de diagnosticar. \\
...
4 & Definir y documentar guías de configuración de datos demo con \texttt{OWNER\_EMAIL} coherente antes de iniciar las sesiones de QA. & Los datos demo mal configurados generaron inconsistencias en el flujo de alertas que inicialmente fueron confundidas con bugs del sistema. \\
...
2 & Usar datos demo sin coherencia de \texttt{OWNER\_EMAIL} para validar flujos que dependen del aislamiento de datos por usuario. & Los datos mal configurados generan comportamientos que simulan bugs inexistentes y desperdician tiempo de diagnóstico del equipo. \\
...
5 & Ignorar el impacto de la navegación con recarga completa de página en la experiencia del usuario final. & La navegación con \texttt{Link} demostró que el cambio a enrutamiento nativo tiene un impacto perceptible en la fluidez del sistema. \\
\end{longtable}
\endgroup
...
2 & Navegar entre vistas del sistema mediante componentes \texttt{Link} en lugar de recargas completas de página. & Mejora la experiencia del usuario, reduce el tiempo de navegación y aprovecha el estado de React entre vistas. \\
...
\subsection{8. Conclusiones de Cierre del Proyecto}
...
\begin{itemize}[leftmargin=1.2em]
\item Del código sin criterios de aceptación (M1) a un Quality Gate de SonarCloud activo y remediado (M4).
\item De la integración sin validación (M1) a un pipeline de CI/CD completo con publicación condicional (M4).
\item De estimaciones optimistas (M1) a Scrum Poker con factor de capacidad real (M3 y M4).
\item De un entorno heterogéneo (M2) a un Docker Compose homogeneizado (M4).
\item De un QA informal (M1) a reportes estandarizados con pasos de reproducción (M3 y M4).
\end{itemize}

El entorno homogeneizado con Docker Compose representa la garantía más sólida de la estabilidad final del sistema: cualquier integrante, en cualquier máquina, puede ejecutar \texttt{docker compose up} y obtener exactamente el mismo entorno. Ese nivel de reproducibilidad es el estándar de la ingeniería de software profesional.

El mayor aprendizaje del proyecto no es técnico: es que la calidad no es un atributo que se añade al final, sino una disciplina que se practica en cada decisión de cada sprint. Desde cuándo crear el \texttt{.env} hasta cómo escribir el mensaje de un commit.
...
\toprule
\textbf{Métrica de cierre} & \textbf{M1} & \textbf{M2} & \textbf{M3} & \textbf{M4} \\
\midrule
\endfirsthead
\toprule
\textbf{Métrica de cierre} & \textbf{M1} & \textbf{M2} & \textbf{M3} & \textbf{M4} \\
...
\toprule
\textbf{Pregunta guía de la PPT} & \textbf{Respuesta aplicada al Milestone 4: Dashboard, Alertas y Cierre del Sistema} \\
\midrule
\endfirsthead
\toprule
\textbf{Pregunta guía de la PPT} & \textbf{Respuesta aplicada al Milestone 4: Dashboard, Alertas y Cierre del Sistema} \\
...
\bottomrule
\endlastfoot
¿Qué producto se produjo? & Se produjo el cierre funcional del sistema: alertas dinámicas SOAT/RTM conectadas a MongoDB Atlas, navegación fluida con \texttt{Link}, refinamiento responsivo, pipeline CI/CD con GitHub Actions, SonarCloud y publicación condicional de imágenes Docker. \\
...
¿Qué lecciones se aprendieron de esta experiencia? & El equipo aprendió que CI/CD no reemplaza validación funcional local, que la cobertura debe crecer durante el desarrollo y que los datos demo deben ser coherentes con \texttt{OWNER\_EMAIL}. \\
...
¿Cómo se deben modificar los procesos? & Hacer obligatoria la validación local antes de PR, crecimiento incremental de pruebas, protocolo \texttt{docker compose down -v} y monitoreo temprano del Quality Gate. \\
¿Cuáles son las áreas de más alta prioridad para analizar? & Cobertura automatizada, pruebas E2E, validación local, datos demo con \texttt{OWNER\_EMAIL}, seguridad SonarCloud y limpieza del entorno Docker. \\
\end{longtable}
\endgroup
...
\begin{longtable}{P{0.17\linewidth}P{0.20\linewidth}P{0.22\linewidth}P{0.22\linewidth}P{0.14\linewidth}}
\toprule
\textbf{Rol} & \textbf{¿Qué funcionó?} & \textbf{¿Dónde estuvieron los problemas?} & \textbf{¿Dónde se puede mejorar?} & \textbf{Objetivo de mejoramiento para proyectos futuros o mantenimiento} \\
\midrule
\endfirsthead
\toprule
\textbf{Rol} & \textbf{¿Qué funcionó?} & \textbf{¿Dónde estuvieron los problemas?} & \textbf{¿Dónde se puede mejorar?} & \textbf{Objetivo de mejoramiento para proyectos futuros o mantenimiento} \\
...
¿Cómo se tuvo en cuenta la usabilidad? & Mejoró con navegación usando \texttt{Link}, menos recargas y ajustes responsivos. \\
...
¿Cómo mejorarlos? & Documentar \texttt{docker compose down -v}, exigir evidencia local y vincular cada cambio a issue/PR con checklist completo. \\
...
Más de & ¿Qué acciones que ya hacemos deberíamos hacer más para mejorar productividad, comunicación, calidad y aprendizaje? & Hacer más colaboración asíncrona, navegación con \texttt{Link}, monitoreo del pipeline y pruebas manuales de flujos críticos. \\
...
Dejar de hacer & ¿Qué acciones deberíamos dejar de hacer porque no son productivas o ya no se necesitan? & Dejar de abrir PR sin validación funcional local, ignorar Hotspots y usar datos sin \texttt{OWNER\_EMAIL} coherente. \\
\end{longtable}
\endgroup
...
Datos demo formales & Inconsistencias por \texttt{OWNER\_EMAIL} & Crear guía y semilla controlada de datos & QA reproduce alertas sin falsos bugs \\
Higiene Docker & Volúmenes contaminan pruebas & Documentar y ejecutar \texttt{docker compose down -v} & Entornos limpios antes de QA \\
\end{longtable}
\endgroup
\end{document}
