...
\begin{document}
\hypersetup{pageanchor=false,pdftitle={Informe de Postmortem - Milestone 2}}
\begin{titlepage}
\centering
\vspace*{0.5cm}
...
{\LARGE \textbf{Milestone 2: Gestión Básica de Flota y Patrones de Diseño}}\\[0.3cm]
...
\clearpage
\section{Postmortem -- Milestone 2: Gestión Básica de Flota y Patrones de Diseño}
...
\item La comunicación sincrónica del equipo disminuyó respecto al M1, pero el rendimiento individual aumentó gracias a la división del trabajo en sub-issues técnicas atómicas en GitHub (un cambio puntual = una issue).
\item Se registraron retrabajos considerables por incompatibilidades de rutas de archivos entre entornos Windows y Linux, especialmente en el uso del alias \texttt{@/} para rutas absolutas en el frontend.
\item Los componentes de Toasts fueron desarrollados antes de que el ToasterLayout base estuviera disponible, generando una dependencia invertida que obligó a rehacer parte del trabajo.
\item El módulo RUNT fue entregado con el simulador de escenarios temporales funcional, cubriendo los tres estados documentales requeridos.
\end{itemize}

\subsubsection{Lecciones aprendidas}

\begin{itemize}[leftmargin=1.2em]
\item La estandarización de convenciones de nombres de archivos no es un detalle menor: conflictos como \texttt{useAlert.js} vs. \texttt{useAlerts.js} generaron confusión en las importaciones y bugs difíciles de rastrear.
\item Construir sobre infraestructura que aún no existe (Toasts sin ToasterLayout) es una forma de deuda técnica anticipada que siempre genera retrabajo.
\item La interoperabilidad entre entornos debe definirse como un requisito técnico del equipo, no como un problema individual de cada desarrollador.
\end{itemize}
...
El Configuration Manager identificó durante este milestone el problema crítico de interoperabilidad entre entornos Windows y Linux. Las rutas de archivos absolutas y el uso del alias \texttt{@/} generaron comportamientos distintos en los entornos de cada desarrollador, causando que código que funcionaba localmente en un entorno fallara en otro. Adicionalmente, la ambigüedad en los nombres de archivos (\texttt{useAlert.js} vs. \texttt{useAlerts.js}) no fue capturada por ningún mecanismo automático, evidenciando la ausencia de linters estrictos. Para el M3, el Configuration Manager debe liderar la configuración de ESLint con reglas de naming conventions y la estandarización del alias \texttt{@/} en todos los entornos del equipo.
...
La causa raíz principal del Milestone 2 fue la \textbf{falta de estandarización en las convenciones de nombres de archivos y en la definición de interoperabilidad entre entornos de desarrollo}. Estas dos condiciones operaron de forma combinada: la ambigüedad en los nombres (\texttt{useAlert} vs. \texttt{useAlerts}) generó bugs de importación silenciosos, mientras que la ausencia de un entorno homogeneizado hizo que los problemas de alias de rutas (\texttt{@/}) se manifestaran de forma inconsistente según el sistema operativo de cada desarrollador.

Como causa secundaria, la \textbf{construcción de componentes visuales sobre infraestructura base inexistente} (Toasts antes que ToasterLayout) evidenció la ausencia de una secuencia de construcción acordada, generando una dependencia invertida que obligó a rehacer trabajo ya completado.
...
\bottomrule
\endlastfoot
1 & Estandarizar las convenciones de nombres de archivos mediante reglas de ESLint configuradas en el repositorio. & Los conflictos entre \texttt{useAlert.js} y \texttt{useAlerts.js} generaron bugs de importación que consumieron tiempo de depuración innecesario. \\
...
3 & Ignorar los conflictos de alias de rutas (\texttt{@/}) entre entornos como si fueran problemas individuales. & Son problemas de equipo que requieren una solución de equipo: un entorno homogeneizado. \\
...
\subsection{8. Plan de Mejora para el Milestone 3}
...
Implementar Docker Compose para homogeneizar el entorno de desarrollo. & DevOps Engineer (@solonlosada2006) & Todos los integrantes ejecutan el proyecto con \texttt{docker compose up} sin configuración adicional. \\
...
\toprule
\textbf{Pregunta guía de la PPT} & \textbf{Respuesta aplicada al Milestone 2: Gestión Básica de Flota y Patrones de Diseño} \\
\midrule
\endfirsthead
\toprule
\textbf{Pregunta guía de la PPT} & \textbf{Respuesta aplicada al Milestone 2: Gestión Básica de Flota y Patrones de Diseño} \\
...
¿Dónde hubo problemas que deben corregirse para el próximo ciclo? & Se deben corregir conflictos de naming, incompatibilidad del alias \texttt{@/}, baja sincronización de integración y desarrollo de Toasts antes del ToasterLayout. \\
...
\toprule
\textbf{Rol} & \textbf{¿Qué funcionó?} & \textbf{¿Dónde estuvieron los problemas?} & \textbf{¿Dónde se puede mejorar?} & \textbf{Objetivo de mejoramiento para el Milestone 3} \\
\midrule
\endfirsthead
\toprule
\textbf{Rol} & \textbf{¿Qué funcionó?} & \textbf{¿Dónde estuvieron los problemas?} & \textbf{¿Dónde se puede mejorar?} & \textbf{Objetivo de mejoramiento para el Milestone 3} \\
...
\bottomrule
\endlastfoot
Estandarizar nombres e importaciones & Bugs por \texttt{useAlert} vs. \texttt{useAlerts} y alias & Configurar ESLint y convención única & Pipeline/linter detecta inconsistencias \\
